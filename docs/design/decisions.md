...
| D-09 | 言語とツール | 決定 (2026-09-25) | — | Python 3.11 以上、uv、ruff、mypy (strict)、pytest。理由: Jev の SDK が Python（F-J7）、Petoi の公式 API も Python（F-B6）。型検査とテストで、実機なしに振る舞いを固められる |
| D-10 | Jev SDK に非同期 API があるか。無い場合の包み方 | 未決定 | フェーズ 1b で SDK を読む。無ければスレッドで包む | |
